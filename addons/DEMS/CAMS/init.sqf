...
_loadVanilla = missionNamespace getVariable "DEMS_CAMS_useVanilla";// Load the vanilla content? 
_loadExile = missionNamespace getVariable "DEMS_CAMS_useExile";
_cartsLoaded = missionNamespace getVariable "DEMS_CAMS_cartList";
...
if (_loadExile) then
{
	_hold = [] execVM "addons\DEMS\CAMS\carts\Exile\assets.sqf";
	if (isNil "_hold") exitWith { diag_log format ["[CAMS:%1] init.sqf: ERROR in assets.sqf",_camsVersion];CAMS_isStable=false;};
	waitUntil { ScriptDone _hold};
	...
	_hold = [] execVM "addons\DEMS\CAMS\carts\Exile\ImmersionFX.sqf";
	if (isNil "_hold") exitWith { diag_log format ["[CAMS:%1] init.sqf: ERROR in cart ImmersionFX.sqf : %2",_camsVersion,_x];CAMS_isStable=false;};
	waitUntil { ScriptDone _hold};
